<script lang="ts">
  ...
  import { updateAsset, type AssetResponseDto, type UpdateAssetDto } from '@immich/sdk';
  ...
  const onClose = async (result?: ChangeLocationResult) => {
    ...
    const updateAssetDto: UpdateAssetDto =
      result.type === 'coordinates'
        ? { latitude: result.point.lat, longitude: result.point.lng }
        : { noLocation: result.value };

    try {
      asset = await updateAsset({ id: asset.id, updateAssetDto });
    } catch (error) {
...
{:else if asset.exifInfo?.noLocation}
  <button
    type="button"
    class="flex w-full text-start justify-between place-items-start gap-4 py-4"
    onclick={() => (isOwner ? (isShowChangeLocation = true) : null)}
  ...
  >
    <div class="flex gap-4">
      <div><Icon icon={mdiMapMarkerOutline} size="24" /></div>
      <p class="text-gray-500 dark:text-gray-400">{$t('no_location')}</p>
    </div>
